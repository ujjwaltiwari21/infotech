...
import { 
  ...
  Phone, 
  MapPin,
  X // Twitter ki jagah ab 'X' use hota hai lucide-react mein
} from "lucide-react";
...
export default function Footer() {
  ...
  return (
    // mt-auto aur w-full bottom space fix karne ke liye
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div className="space-y-6">
            ...
            <p className="text-white/40 leading-relaxed text-sm max-w-xs">
              Premium security solutions and IT infrastructure. Building trust through advanced technology.
            </p>
          ...
          <div>
            ...
            <ul className="space-y-3 text-white/40 text-sm">
              {quickLinks.map((link, i) => (
                <li key={i}>
                  <a href={link.href} className="hover:text-blue-500 transition-all uppercase font-semibold tracking-wider text-[11px]">
                    {link.name}
                  </a>
                </li>
              ))}
            ...
            <div className="flex flex-col gap-3">
              <div className="flex gap-2">
                <input 
                  type="email" 
                  placeholder="Email" 
                  className="bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-sm w-full outline-none focus:border-blue-500/50 transition-all" 
                />
                <button className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg text-white font-bold text-sm transition-colors">
                  Go
                </button>
              </div>
              <p className="text-[10px] text-white/20 uppercase tracking-tighter">Stay updated with latest security tech.</p>
            </div>
          </div>
        </div>
